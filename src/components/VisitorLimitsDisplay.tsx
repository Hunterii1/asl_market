...
interface VisitorLimitsDisplayProps {
  className?: string;
}

export function VisitorLimitsDisplay({ className }: VisitorLimitsDisplayProps) {
  ...
  const fetchLimits = async () => {
    try {
      ...
      // Only extract visitor limits
      if (dailyLimits?.visitor_limits) {
        setLimits({
          license_type: dailyLimits.license_type,
          visitor_limits: dailyLimits.visitor_limits,
          date: dailyLimits.date
        });
      } else {
        setError('محدودیت ویزیتورها یافت نشد');
      }
    } catch (err: any) {
  ...
  const isLimitReached = limits.visitor_limits.remaining <= 0;
        ...
        {isLimitReached && (
          <Alert>
            ...
            <AlertDescription className="text-sm">
              محدودیت روزانه مشاهده ویزیتورها شما به پایان رسیده است. برای افزایش محدودیت، لایسنس پرو تهیه کنید.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
