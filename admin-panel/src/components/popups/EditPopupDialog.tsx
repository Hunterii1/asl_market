...
import { Loader2, Megaphone, FileText, Layout, Calendar, Users, Link, Palette, Clock } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { editPopupSchema, type EditPopupFormData } from '@/lib/validations/popup';
...
// Mock API function
const updatePopup = async (data: EditPopupFormData): Promise<void> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (Math.random() < 0.1) {
        ...
        const index = popups.findIndex((p: Popup) => p.id === data.id);
        if (index !== -1) {
          popups[index] = { ...popups[index], ...data };
          popups[index].updatedAt = new Date().toLocaleDateString('fa-IR');
...
export function EditPopupDialog({ open, onOpenChange, popup, onSuccess }: EditPopupDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [currentTab, setCurrentTab] = useState('basic');

  const form = useForm<EditPopupFormData>({
    resolver: zodResolver(editPopupSchema),
    defaultValues: {
      id: '',
      title: '',
      content: '',
  ...
  useEffect(() => {
    if (popup && open) {
      form.reset({
        ...
        showToUsers: popup.showToUsers,
        specificUserIds: popup.specificUserIds || [],
        ...
        displayCount: popup.displayCount || 0,
        clickCount: popup.clickCount || 0,
      });
      setCurrentTab('basic');
    }
  }, [popup, open, form]);
  ...
  const onSubmit = async (data: EditPopupFormData) => {
    setIsSubmitting(true);
    try {
      await updatePopup(data);
      toast({
        title: 'موفقیت',
        description: 'اطلاعات پاپ‌آپ با موفقیت به‌روزرسانی شد.',
        variant: 'default',
      });
      onOpenChange(false);
      onSuccess?.();
